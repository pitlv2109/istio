...
type (
	...
	Metric interface {
		...
		// Reset a single tag value to 0. This method should only be used for LastValue.
		ResetTagValue(tagValue TagValue)

		// Reset all tag values of a measurement to 0. This method should only be used for LastValue.
		ResetAllTagValues()

		// Delete all instances of tagValue.
		DeleteTagValue(tagValue TagValue)
...
func MustRegisterViews(metrics ...Metric) {
	for _, m := range metrics {
		fmt.Println("REGISTER VIEW " + m.Name())
...
func UnregisterViews(metrics ...Metric) {
	for _, m := range metrics {
		fmt.Println("DELETE VIEW " + m.Name())
...
func (f *float64Metric) ResetTagValue(tagValue TagValue) {
	if f.view.Aggregation.Type != view.AggTypeLastValue {
		log.Warnf("should reset tag value for LastValue type only. Found type %v",
			f.view.Aggregation.Type)
	}
	f.WithTags(tagValue).Record(0)
}

func (f *float64Metric) DeleteTagValue(value TagValue) {
	for i, tagValue := range f.tagValues {
		if value == tagValue {
			f.tagValues[i] = f.tagValues[len(f.tagValues)-1]
			f.tagValues = f.tagValues[:len(f.tagValues)-1]
			i--
		}
	}
}
